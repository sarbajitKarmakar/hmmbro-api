import pool,
{
    ...
    deleteImageQuery,
    getProductVariantQueryToCheck,
    disprimaryAllImageQuery,
} from '../model/db.js';
...
export const updateProductService = async (req) => {
    ...
    try {
        ...
        if (!updatedProductVariant) throw new Error("Product Variant not found");
        ...
        if (imageDetails) {
            ...
            const deletedPrimary = deletedImageDetails.some(
                img => img.isprimary === true
            );

            if (deletedPrimary && !imageDetails.primary_image_type) {
                throw new Error(
                    "You must select a new primary image after deleting the current primary."
                );
            }
            ...
            if (imageDetails.primary_image_type === 'existing' && imageDetails.primary_existing_id) {
                await makeImagePrimaryQuery(id, imageDetails.primary_existing_id);
            }
        ...
        if (req.files) {
            ...
            if (!defaultPrimaryImg) {
                const isAvailable = await getProductVariantQueryToCheck(client, id);
                if (isAvailable) throw new Error("Product Variant not found");
            }
            const imgPayload = [];
            let i = 0;
            for (const file of req.files) {
                ...
                if (imageDetails.primary_image_type === 'new' && Number(imageDetails.primary_new_index) === i) {
                    defaultPrimaryImg = true;
                    await disprimaryAllImageQuery(client, id); //to set every isprimary = false for new primary image
                }
